...
def train(modelt, optimizert, criteriont, tt, targett, stimt, 
    ...
    for epoch in range(tt): 
        ...
        toprint['R_l0inp'] = reg0in
        toprint['R_l0rec'] = reg0rec
        toprint['R_l1inp'] = reg1in
        toprint['R_l1rec'] = reg1rec
        toprint['R_out'] = reg1out
        toprint['R_l0binp'] = reg0bin
        toprint['R_l0brec'] = reg0brec
        toprint['R_l0bout'] = reg0bout
        # term 2: rates
        ...
        toprint['R_l0brate'] = reg0bact
        loss = loss_train+reg1in+reg1rec+reg0in+reg0rec+reg1out+reg1act+ \
                reg0act+reg0bin+reg0brec+reg0bout+reg0bact
        loss.backward()
